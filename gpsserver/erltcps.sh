...
# Default-Stop:      0 1 6
# Short-Description: Germes TCP server
# Description:       Germes TCP server
### END INIT INFO
...
case "$1" in 
    start) 
        log_daemon_msg "Starting erlang daemon" 
        do_start 
        log_end_msg $? 
        ;; 
    stop) 
        log_daemon_msg "Stop erlang daemon" 
        do_stop 
        log_end_msg $? 
        ;; 
    status) 
        status_of_proc "$DAEMON" "Erlang daemon" && exit 0 || exit $? 
        ;; 
	remsh)
		do_rmshell
		;;
	libupdate)
		...
		;;
    restart) 
        log_daemon_msg "Restarting erlang daemon" 
        do_stop 
        case "$?" in 
            0|1) 
                do_start 
